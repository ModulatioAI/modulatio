...
> [!WARNING]
> **v0.7.2 — Beta.** **Conversation-first: you resolve work by talking to the Leader.** Approvals move into the LEADER chat — the Tickets tab is now a read-only audit log, and you approve by telling the Leader ("approve the budget", "yes, go ahead"); he carries it out. The old PLANS tab is now a **JT Library** (browse the Job-Template library), and you can **attach a document or image** to the Leader chat. A user-editable **constitution** (honesty / diligence / harm-avoidance / respect) shapes how the Leader talks with you — injected only into his conversational persona. And **`/bug`** files a GitHub issue with a redacted diagnostics bundle (or opens a prefilled URL with no token). Builds on v0.7.1's pool-by-default keys + Configuration tab, v0.6.0's routing-reality, v0.5.0's Job Templates, and the skill-library keystone. See the [CHANGELOG](CHANGELOG.md) for the full delta. Cleared fresh **hull + coherence** reviews. **2638 tests pass.** Read the [Beta calibration](https://modulatio.ai/v0-1-0-beta/) page before kicking off serious work — knowing the engine's real ceilings is the difference between a smooth run and a frustrated one. Bug reports + discussions welcome on the [issues tab](https://github.com/ModulatioAI/modulatio/issues) and [discussions](https://github.com/ModulatioAI/modulatio/discussions).

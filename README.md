...
> [!WARNING]
> **v0.7.1 — Beta.** **Configure the whole team from the TUI, and your keys finally work the way you'd expect.** A menu-driven **Configuration tab** wires providers, models, API keys, and agents — pick a provider and a model and everything auto-fills; you type only a key. **Keys are a per-provider shared pool by default** (every model rotates across them — throughput + 429 failover); **pin** a key to a model when you want its spend isolated for a budget, and it leaves the pool. A standalone **Providers & keys** manager lists and removes keys (by label, never the value). You can also **talk to the Leader** directly now — ask him to answer, fetch the web, author a skill, or *run a job* and command the swarm. And the agent team has a name: **The Mod Squad**. Builds on v0.6.0's routing-reality + role-language migration, v0.5.0's Job Templates, v0.4.0's **skill self-codification**, and v0.3.0's **skill-library keystone**. See the [CHANGELOG](CHANGELOG.md) for the full delta. Cleared fresh **hull + coherence** reviews. **2648 tests pass.** Read the [Beta calibration](https://modulatio.ai/v0-1-0-beta/) page before kicking off serious work — knowing the engine's real ceilings is the difference between a smooth run and a frustrated one. Bug reports + discussions welcome on the [issues tab](https://github.com/ModulatioAI/modulatio/issues) and [discussions](https://github.com/ModulatioAI/modulatio/discussions).

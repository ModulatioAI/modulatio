...
> [!WARNING]
> **v0.8.4 — Beta.** **Verify the whole deliverable against the brief, not just the parts.** A run could assemble eight real parts into one product and still ship it bare — no title or table of contents, parts mis-numbered so eight read like nine, several under the brief's per-part length floor — and both QC and the lead's verify passed it. Everything the engine hardened verified the *parts* and the *plumbing*; nothing verified the *product* against the brief. v0.8.4 builds that missing organ: the verifier gets **eyes** (an engine-extracted structural *digest* + a readable text *twin*, so the whole is judged on what it actually *is*, never on binary bytes the model can't read — a deliverable the engine can't read can no longer ship clean); a declared **`DeliverableSpec`** carries the per-part floor / required structure / title from the job template into the run and a deterministic check runs the whole against it at verify; the engine **binds the floor at produce** (the cheap producer is held to it upstream, on the assembler's *real* part set — never a front-matter page); and it **produces the framing** (title + table of contents) and **normalizes part numbering** to a clean 1..N. Every product-specific move is a per-family dispatch — document-first, every other family a graceful no-op, product- *and* agent-agnostic. Builds on v0.8.2's assembler families + metered tier. See the [CHANGELOG](CHANGELOG.md) for the full delta. Cleared fresh **hull + coherence** reviews (a real over-stamp bug + a false smoke command found and sealed). **2993 tests pass.** Read the [Beta calibration](https://modulatio.ai/v0-1-0-beta/) page before kicking off serious work — knowing the engine's real ceilings is the difference between a smooth run and a frustrated one. Bug reports + discussions welcome on the [issues tab](https://github.com/ModulatioAI/modulatio/issues) and [discussions](https://github.com/ModulatioAI/modulatio/discussions).

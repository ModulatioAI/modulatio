...
> [!WARNING]
> **v0.9.6 — Beta. The team always finishes the job — plus lifecycle tooling and a leaner team.** The headline this release is reliability: a producer's attempts are now budgeted **per task** — across every retry, hand-off, and re-run — so a model can't loop forever or quietly skirt the quality gate. When that budget is spent, **QC finishes the work itself** — patching the existing draft, or writing the artifact from the task's brief when there's nothing to patch — so a run lands a real deliverable instead of wedging. You also get clearer signals: each **QC review shows up in the activity feed** against its task (you can see the work was checked, not just that a producer "wrapped up"), and if the **Leader's model is unavailable** when you start a job, Modulatio tells you plainly to switch it instead of failing with a stack trace. The team is leaner — the **Leader is the only required role** (add a QC verifier and producers as you want them) — and you can keep an install tidy with **`modulatio uninstall`** (tiered, backed-up) and **`modulatio repair`**. Talk to the Leader with **`/models`**, **`/new`**, **`/editor`**, and stop it mid-thought with **ESC**. Security: a **Clay** subscription seat (`claude -p`) running as a producer/QC is now **fail-closed confined** — a fixed non-process toolset, customizations off, no shell or sub-agent spawners. Every change cleared a four-lens cadre review (security, hull/terminal-state, code-quality, coherence). Builds on **v0.9.4** (the two-lane Leader: a standalone coding agent + `/yolo` `/goal` `/yolo-goal` modes) and **v0.9.3** (Feng-Tui reskin). **4733 tests pass.** See the [CHANGELOG](CHANGELOG.md) for the full delta and the [roadmap](https://modulatio.ai/roadmap/) for what's next (1.0: a web UI + remote access). Read the [Beta calibration](https://modulatio.ai/v0-1-0-beta/) page before serious work. Bug reports + discussions welcome on the [issues tab](https://github.com/ModulatioAI/modulatio/issues) and [discussions](https://github.com/ModulatioAI/modulatio/discussions).

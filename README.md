...
> [!WARNING]
> **v0.9.8.9 — Beta. No stalled call outlives its clock.** When a model call **spins or stalls in a way no network timeout can see** — the live 17-minute wedge was a tool-loop completion burning CPU in silence — it is now **force-released** 30s past its transport timeout. The seat fails as an **availability error**, so the same self-healing you already had takes over (fallback chain, backoff, seat cooldown, and QC as the producer of last resort), and the wedged call's **all-threads stack dump** lands in the crash log so the next wedge leaves evidence instead of a mystery. The tool-loop's transport bound now honors `MODULATIO_CALL_TIMEOUT` (default 600s) like every other call — it was a hardcoded 1800s nothing overrode, the exact seam that wedged; raise the knob if your local models legitimately need longer. Plus: **assembled reports get a real title** in ad-hoc runs (derived from the plan's own output name, instead of opening with the first section's heading — which delivery was also inheriting as the export name), and the **boot splash drops its stray µ block-art**. Builds on **v0.9.8.8** (the live telemetry rail + self-healing team) and **v0.9.8.7** (research reuse + context-size fan). **5105 tests pass.** See the [CHANGELOG](CHANGELOG.md) for the full delta and the [roadmap](https://modulatio.ai/roadmap/) for what's next (1.0: a web UI + remote access). Read the [Beta calibration](https://modulatio.ai/v0-1-0-beta/) page before serious work. Bug reports + discussions welcome on the [issues tab](https://github.com/ModulatioAI/modulatio/issues) and [discussions](https://github.com/ModulatioAI/modulatio/discussions).

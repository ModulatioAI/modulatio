...
> [!WARNING]
> **v0.5.0 — Beta.** Every job now gets its **own output folder**, and recurring work gets **Job Templates** — the Leader's own self-authored interview, parameter schema, and output contract for a *class* of job, so a weekly brief, an N-piece anthology, or a standing research report runs the same way every time (headless from cron when you want it), each run's deliverables landing in their own named folder instead of clobbering the last. It's the **setup-side of the Alfred loop**: where v0.4.0 codifies recurring QC *failures* into skills, v0.5.0 codifies recurring *setup-gaps and redos* into templates the team reuses — and notices on its own when you keep running the same kind of job. Builds on v0.4.0's **skill self-codification** (the team learns from its own failures into git-versioned skill guidance), v0.3.0's **skill-library keystone** (a producer is a **model endpoint**; dispatch routes on **capability + availability and never blocks**), and the v0.2.x **QC-thesis arc** (cheap producers generate the bulk; the smarter QC patches only the errors; the **Product Quality Report** ships honest caveats, never a gate). See the [CHANGELOG](CHANGELOG.md) for the full delta. Validated end-to-end on live heterogeneous model lineups and cleared fresh two-reviewer audits. **2522 tests pass.** Read the [Beta calibration](https://modulatio.ai/v0-1-0-beta/) page before kicking off serious work — knowing the engine's real ceilings is the difference between a smooth run and a frustrated one. Bug reports + discussions welcome on the [issues tab](https://github.com/ModulatioAI/modulatio/issues) and [discussions](https://github.com/ModulatioAI/modulatio/discussions).

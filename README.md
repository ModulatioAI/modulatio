...
> [!WARNING]
> **v0.9.8.8 — Beta. Watch the run, and trust the team to finish it.** The factory floor's telemetry rail is now **live** — a run's elapsed time, a **tasks bar with a real percentage**, the QC pass/reject tally, the context load (tokens + compressions), and **every producer's current move** (the same icon + verb vocabulary as the stream), all painted as the run moves; QC even steps onto the floor while it reviews. The team **finishes the job even when a seat dies**: a crashed local model (an OpenAI-compatible endpoint reports it as an HTTP 400) is recognized as an **availability failure** — its fallback chain engages, retries **back off** instead of burning a budget in a second, and the seat is **cooled out of the dispatch pool** so it stops attracting tasks. And when a goal's work is incomplete with the retry budget spent, **QC — the producer of last resort — authors the missing pieces** from each task's brief, in dependency order, rather than shipping a goal with a hole in it (proven by a live kill-test: a model killed mid-run, the team healed and shipped satisfied with zero human touches). The seats now **tell the truth about themselves**: a producer wearing a reasoning model that can't be quieted through its endpoint is flagged by `modulatio doctor`, the agent builder, and the run log — so runaway reasoning tokens stop being a surprise you find in the compression logs (Qwen's in-band `/no_think` toggle rides the message text, which no shim can drop). Plus **agents accrue episodic memory** from the jobs they run (surfaced in MEMORY, with QC memory proposals you approve or reject), a new **SETTINGS** tab tunes the engine's knobs from the TUI, the **CONSOLE** flip is now two real **LEADER / MOD SQUAD** tabs (launching a job no longer yanks you to the floor), and the activity feed **speaks in icons** with two more Feng-Tui variants (red + purple) on the F2 cycle. Builds on **v0.9.8.7** (research reuse + context-size fan) and **v0.9.8.6** (single-source leadership). **5088 tests pass.** See the [CHANGELOG](CHANGELOG.md) for the full delta and the [roadmap](https://modulatio.ai/roadmap/) for what's next (1.0: a web UI + remote access). Read the [Beta calibration](https://modulatio.ai/v0-1-0-beta/) page before serious work. Bug reports + discussions welcome on the [issues tab](https://github.com/ModulatioAI/modulatio/issues) and [discussions](https://github.com/ModulatioAI/modulatio/discussions).

...
> [!WARNING]
> **v0.8.0 — Beta.** **Talk to the Leader from your editor.** Modulatio now ships an **Agent Client Protocol (ACP) server** — `modulatio acp --code <project>` exposes the conversational Leader over JSON-RPC-on-stdio, so a Zed-class editor can drive him: prompt turns, live activity, and **client-approved tool calls** (the Leader asks before it touches anything; reject and it re-plans). It's the same Leader and the same per-project memory as the TUI — a turn over ACP and a turn in the TUI continue one conversation. The redundant **STATUS tab** is gone (the LEADER / MOD SQUAD TV already streams the activity). Builds on v0.7's conversation-first move (approvals via the Leader, JT Library, attachments, constitution, `/bug`) and pool-by-default keys + the Configuration tab. See the [CHANGELOG](CHANGELOG.md) for the full delta. Cleared fresh **hull + coherence** reviews; ACP validated live against a real model (a conversation + a kickoff end-to-end). **2648 tests pass.** Read the [Beta calibration](https://modulatio.ai/v0-1-0-beta/) page before kicking off serious work — knowing the engine's real ceilings is the difference between a smooth run and a frustrated one. Bug reports + discussions welcome on the [issues tab](https://github.com/ModulatioAI/modulatio/issues) and [discussions](https://github.com/ModulatioAI/modulatio/discussions).

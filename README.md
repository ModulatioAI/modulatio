...
> [!WARNING]
> **v0.8.9 — Beta. Security hardening.** A full-codebase security audit of the agent engine, then **two independent mirror-audits** — an adversarial *hull* pass and a *coherence* pass, each a different model reviewing the whole tree fresh — to catch what one pass would miss. **Nine findings confirmed and closed**, the most important (a tool-call authorization bypass that let a skill reach a tool outside its declared loadout) found by the *independent* pass, not the first. Highlights: dispatch now respects each skill's `tool_loadout`; skill/job-template names can't escape their registry (no traversal / poisoning); front-matter can't forge a privilege; `run_shell` is contained (resource limits, process-group reaping, an opt-in fail-closed sandbox via `MODULATIO_REQUIRE_SANDBOX=1`); secrets are redacted before they hit alerts / checkpoints / the conversation log; ACP attachments are confined to an allowed root; tool timeouts are clamped. **No functional change for a normal run** — this is defense-in-depth on the surfaces a prompt-injected model could otherwise reach; every fix is an engine-bound invariant, not prompt guidance. The guiding rule: *a permission is a key to a door inside the ship; it never opens the sea valves.* See the [CHANGELOG](CHANGELOG.md) for the full per-finding delta and the [roadmap](https://modulatio.ai/roadmap/) for what's next (0.9.0: bug/crash reporting + performance; 1.0: a new TUI + a web UI + remote access). Builds on **v0.8.8** (deterministic assembly validation + codify-the-win). **3211 tests pass.** Read the [Beta calibration](https://modulatio.ai/v0-1-0-beta/) page before serious work. Bug reports + discussions welcome on the [issues tab](https://github.com/ModulatioAI/modulatio/issues) and [discussions](https://github.com/ModulatioAI/modulatio/discussions).

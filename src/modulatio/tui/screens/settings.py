...
class SettingsScreen(Vertical):
    ...
    def _source_of(self, key: str) -> str:
        return settings_knobs.knob_source(key)

    def _value_of(self, key: str) -> str:
        return settings_knobs.knob_value(key)
    ...
    def _save_value(self, key: str, raw: str) -> bool:
        """Validate + persist + live-apply one knob via the shared seam. False
        = refused (the seam holds the shell/.env guard + range checks)."""
        ok, reason = settings_knobs.set_knob(key, raw)
        if not ok:
            self.app.notify(reason.capitalize(), severity="warning")
            return False

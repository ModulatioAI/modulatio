...
def build_metered_authorizer(
    *,
    ...
    def authorize(called_name: str, args: dict) -> "tuple[bool, str]":
        ...
        bad = sorted(k for k in args if k.lower() in FORBIDDEN_ARG_KEYS)
        if bad:
            return (
                False,
                f"metered tool {called_name!r}: forbidden arbitrary-network "
                f"param(s) {bad} — narrow params (pinned artifacts + options) only",
            )
        ...
        # 3. Idempotency key over pinned inputs + tool + (non-forbidden) options.
        options = {k: v for k, v in args.items() if k.lower() not in FORBIDDEN_ARG_KEYS}
        key = metered_idempotency_key(tool_name, checksums, options)

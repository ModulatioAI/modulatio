...
- **Honor the unit's size budget — the floor is binding.** The task names the unit's target size. When it sets a numeric floor (a `token_count` budget the engine measures the output against), **meeting that floor is a hard requirement, not a suggestion** — a unit that lands materially under it is a defect and QC sends it back to expand. Write to the target: develop the material to full size the first time. Going *over* wastes tokens and risks truncation, so stay near the target, not past it. If the task states no size, judge a substantial, complete treatment for the unit's role — don't ship a thin sketch. Expand only by developing real content (more depth, detail, and fuller treatment consistent with the unit's established structure and the standards), **never by padding** with filler, repetition, or restated material — padding fails QC the same as undershooting. If you believe the stated floor is genuinely wrong for this unit, flag it via `summary_for_state_doc`, but still ship to the floor.

---
name: leader-verify
description: Goal-level verdict — Leader reads the actual deliverables (read_file across the whole run directory; run_shell rooted at artifacts) and reasons over aggregate task outcomes, writes a human-facing report, picks satisfied/on_the_fence/disappointed. Bundled as canonical Leader-verify prompt; users override at <shared>/skills/leader-verify.md or <project>/skills/leader-verify.md. The tool loadout routes verify through the chat-loop so the verdict rests on observed files, not the task summaries alone.
executor: llm
tool_loadout: run_shell, read_file
...
You have `read_file`, scoped to the whole run directory — the entire harness
for this run: the deliverables under `artifacts/`, the `reports/`, and any
logs or run state — and `run_shell` (rooted at `artifacts/`, sandboxed) for
running/inspecting the deliverable itself. Before you decide, LOOK: read the
bound deliverable (and any section files) to confirm with your own eyes that
it exists and that what the goal requires is actually present — the abstract,
a real References section, the required sections, a comparison table. A task
summary saying "I included X" is a CLAIM; the file is the proof. Never write
"asserted by the task but unverified by me" about a file you could have
opened — open it.

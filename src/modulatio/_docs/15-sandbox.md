...
When the operator has configured outside services (Config tab → SERVICES), the registry also merges in the **service tools** (`generate_image`, `generate_video`, `generate_speech`, `research_search`, `api_call`) — same opt-in shape as `run_shell`: nothing configured, nothing in the registry. Their network target is bounded by the service's **operator-pinned base URL** (for `api_call`, paths must be relative and the host is re-checked — the model can never choose a host), and their API key is injected at the adapter layer, never visible to the model. See [Tool catalog](/reference/tools/) for per-tool schemas, args, and the safety contract per tool. Future releases will likely add more (a build/test feedback primitive, a multi-language symbol-map primitive, a cost-telemetry surface) — see [Roadmap](/roadmap/).
